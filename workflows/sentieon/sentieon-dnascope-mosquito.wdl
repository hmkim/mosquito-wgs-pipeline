...
task SentieonDedupAndCall {
  ...
  command <<<
    ...
    sentieon driver \
      ...
      ~{sample_id}.dedup.~{if output_cram then "cram" else "bam"}
    ...
    DEDUP_FILE=~{sample_id}.dedup.~{if output_cram then "cram" else "bam"}
    ...
    if [ "~{output_cram}" = "true" ]; then
      sentieon util index $DEDUP_FILE
      mv $DEDUP_FILE ~{sample_id}.cram
      mv ${DEDUP_FILE}.crai ~{sample_id}.cram.crai
    else
      mv $DEDUP_FILE ~{sample_id}.bam
      mv ${DEDUP_FILE}.bai ~{sample_id}.bam.bai
    fi
