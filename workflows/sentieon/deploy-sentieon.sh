...
#   WDL_VERSION   v4 (default, single-task) or v1 (original two-task)
#   BUCKET        S3 data bucket (default: mosquito-wgs-data-<ACCOUNT_ID>-<REGION>)
#   AWS_DEFAULT_REGION  target region (default: ap-southeast-1)
# ─────────────────────────────────────────────────────────
...
BUCKET="${BUCKET:-mosquito-wgs-data-${ACCOUNT_ID}-${REGION}}"
PROJECT_TAG="mosquito-wgs"
SENTIEON_VERSION="${SENTIEON_VERSION:-202503.03}"

LICENSE_STACK="mosquito-wgs-sentieon-license"
LICENSE_TEMPLATE="${PROJECT_ROOT}/cloudformation/sentieon-license-server-stack.yaml"
ECR_REPO="mosquito-wgs-sentieon"
ECR_TAG="omics-1"
WORKFLOW_ZIP="${SCRIPT_DIR}/sentieon-dnascope-workflow.zip"
OMICS_ROLE_NAME="mosquito-wgs-omics-sentieon-role"
OMICS_CONFIG_NAME="mosquito-wgs-sentieon-vpc"

# WDL version to deploy. v4 is the recommended single-task workflow
# (109 min / $3.41 per sample); v1 is the original two-task workflow.
WDL_VERSION="${WDL_VERSION:-v4}"
case "${WDL_VERSION}" in
    v4)
        WDL_NAME="sentieon-dnascope-mosquito-v4.wdl"
        RUN_INPUTS="${SCRIPT_DIR}/run-inputs-SRR6063611-v4.json"
        ;;
    v1)
        ...
        ;;
    *)
        echo "[ERROR] Unsupported WDL_VERSION '${WDL_VERSION}' (expected v1 or v4)" >&2
        exit 1
        ;;
esac
WORKFLOW_NAME="sentieon-dnascope-mosquito-${WDL_VERSION}"
WORKFLOW_ID_FILE="${SCRIPT_DIR}/.workflow-id.${WDL_VERSION}"
...
NEW_VPC_CIDR="${NEW_VPC_CIDR:-10.0.0.0/16}"
...
show_help() {
    ...
    echo "  NEW_VPC_CIDR      CIDR for new VPC (default: 10.0.0.0/16)"
    echo "  SENTIEON_VERSION  Sentieon version (default: 202503.03)"
    echo "  WDL_VERSION       v4 single-task (default) or v1 two-task"
    echo "  BUCKET            S3 data bucket (default: mosquito-wgs-data-<ACCOUNT_ID>-<REGION>)"
    echo ""
    echo "Active configuration:"
    echo "  Region:           ${REGION}"
    echo "  WDL version:      ${WDL_VERSION} (${WDL_NAME})"
...
deploy_license_stack() {
    ...
    echo ""
    echo " NEXT STEP: Send this IP to Sentieon Support to request a license"
    echo "   Portal: https://support.sentieon.com"
    echo "   Host:  ${LICENSE_IP}"
    echo "   For:   mosquito-wgs project, account ${ACCOUNT_ID}, ${REGION}"
    echo "============================================"
}
