#!/usr/bin/env Rscript
# Mosquito WGS Stochastic Simulation Runner
...
cat(sprintf("=== Mosquito WGS Simulation Runner ===\n"))
cat(sprintf("Mode: %s | Chain: %d | Iterations: %d\n", run_mode, chain_index, iterations))

# --- Database Connection ---
get_db_connection <- function() {
  secret_json <- system("aws secretsmanager get-secret-value --secret-id mosquito-wgs-poc/rds-master --query SecretString --output text", intern = TRUE)
...
run_single_simulation <- function(params, seed = NULL) {
  ...
  # Simulation loop (placeholder — actual ODE/stochastic logic from Research team)
  results <- list()
