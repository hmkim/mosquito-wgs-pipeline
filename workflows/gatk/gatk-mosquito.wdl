...
## GATK Mosquito Whole-Genome Sequencing Pipeline
## Target: Aedes aegypti (AaegL5 reference genome)
## Platform: AWS HealthOmics
## Reference: Nature Communications 2025, doi:10.1038/s41467-025-62693-y
...
workflow GatkMosquitoPipeline {
  input {
    ...
    File reference_bwt_2bit_64
    File reference_0123
    File reference_ann
    File reference_amb
    File reference_pac
    String docker_image = "<ACCOUNT_ID>.dkr.ecr.<REGION>.amazonaws.com/mosquito-wgs-gatk:latest"
  }
  ...
  call BwaAlign {
    input:
      ...
      reference_bwt_2bit_64 = reference_bwt_2bit_64,
...
task BwaAlign {
  ...
  command <<<
    # BWA-mem2 expects index files alongside the reference fasta.
    # HealthOmics localizes each file to a read-only path, so we
    # stage everything into a writable directory with correct naming.
    REF_DIR=/tmp/ref
    mkdir -p ${REF_DIR}
    REF_BASE=$(basename ~{reference_fasta})

    ln -s ~{reference_fasta} ${REF_DIR}/${REF_BASE}
    ln -s ~{reference_bwt_2bit_64} ${REF_DIR}/${REF_BASE}.bwt.2bit.64
    ln -s ~{reference_0123} ${REF_DIR}/${REF_BASE}.0123
...
task HaplotypeCaller {
  ...
  command <<<
    # Stage reference files in a writable directory (HealthOmics
    # localizes inputs to read-only paths).
    REF_DIR=/tmp/ref
    mkdir -p ${REF_DIR}
    REF_BASE=$(basename ~{reference_fasta})
    ...
    # Symlink BAM index alongside BAM
