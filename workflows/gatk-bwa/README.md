...
| Docker image | `mosquito-wgs-gatk:latest` | `mosquito-wgs-gatk-bwa:latest` |
...
aws ecr create-repository --repository-name mosquito-wgs-gatk-bwa --region <REGION>
aws ecr get-login-password --region <REGION> | \
  docker login --username AWS --password-stdin <ACCOUNT_ID>.dkr.ecr.<REGION>.amazonaws.com
docker build -t mosquito-wgs-gatk-bwa:latest .
docker tag mosquito-wgs-gatk-bwa:latest <ACCOUNT_ID>.dkr.ecr.<REGION>.amazonaws.com/mosquito-wgs-gatk-bwa:latest
docker push <ACCOUNT_ID>.dkr.ecr.<REGION>.amazonaws.com/mosquito-wgs-gatk-bwa:latest

# 2. Grant HealthOmics ECR access (same policy as before)
aws ecr set-repository-policy --repository-name mosquito-wgs-gatk-bwa \
