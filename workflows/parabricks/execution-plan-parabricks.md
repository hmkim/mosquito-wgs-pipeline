...
|---|---|---|
| S3 Bucket | ✅ Ready | `mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2` |
...
| IAM Role | ✅ Updated | `mosquito-wgs-omics-workflow-role` includes `parabricks` ECR access |
| HealthOmics Workflows | ✅ Registered | v4 (1418762), v5 (2351613), v6 (6693313), v6b (5652212) |
| EC2 Instance | ✅ Available | `<EC2_INSTANCE_ID>` (r5.4xlarge) |
...
aws s3 cp s3://mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2/reference/mosquito/AaegL5/AaegL5.fasta.bwt .
aws s3 cp s3://mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2/reference/mosquito/AaegL5/AaegL5.fasta.sa .
...
aws s3 cp AaegL5.fasta.tar \
    s3://mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2/reference/mosquito/AaegL5/AaegL5.fasta.tar
```
...
docker tag nvcr.io/nvidia/clara/nvidia_clara_parabricks_amazon_linux:4.3.1-1 \
    ACCOUNT_ID.dkr.ecr.ap-northeast-2.amazonaws.com/parabricks:4.3.1-1
docker push ACCOUNT_ID.dkr.ecr.ap-northeast-2.amazonaws.com/parabricks:4.3.1-1
```
...
The existing IAM role only allows ECR pull from `mosquito-wgs-gatk`. Add the new `parabricks` repository.

```bash
# Get current policy
aws iam get-role-policy \
    --role-name mosquito-wgs-omics-workflow-role \
        ...
        "Statement": [
            {
                ...
                "Resource": [
                    "arn:aws:s3:::mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2",
                    "arn:aws:s3:::mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2/*"
                ]
            },
            {
            ...
            },
            {
                ...
                "Resource": [
                    "arn:aws:ecr:ap-northeast-2:ACCOUNT_ID:repository/mosquito-wgs-gatk",
                    "arn:aws:ecr:ap-northeast-2:ACCOUNT_ID:repository/parabricks"
                ]
            },
            {
                "Sid": "CloudWatchLogs",
                ...
                "Resource": "arn:aws:logs:ap-northeast-2:ACCOUNT_ID:log-group:/aws/omics/*"
            }
        ]
    }'
```
...
```bash
cd /home/ec2-user/project-mosquito-wgs/workflows/parabricks
...
```json
{
  "ParabricksGermline.inputFASTQ_1": "s3://mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2/raw/mosquito-wgs/SRR6063611/SRR6063611_R1.fastq.gz",
  "ParabricksGermline.inputFASTQ_2": "s3://mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2/raw/mosquito-wgs/SRR6063611/SRR6063611_R2.fastq.gz",
  "ParabricksGermline.inputRefTarball": "s3://mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2/reference/mosquito/AaegL5/AaegL5.fasta.tar",
  "ParabricksGermline.sample_id": "SRR6063611",
  "ParabricksGermline.pb_version": "4.3.1-1",
  "ParabricksGermline.ecr_registry": "ACCOUNT_ID.dkr.ecr.ap-northeast-2.amazonaws.com"
}
```
...
aws omics start-run \
    ...
    --output-uri s3://mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2/omics-output/ \
...
```bash
# 1. Download output
aws s3 cp s3://mosquito-wgs-data-ACCOUNT_ID-ap-northeast-2/omics-output/<RUN_ID>/out/ \
    ./pb_output/ --recursive
